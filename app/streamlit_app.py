...
from src.predict import predict_single, predict_batch, DEFAULT_THRESHOLD
...
# PAGE 5 — A/B TEST SIMULATION
elif page == "🧪 A/B Test Simulation":
    st.title("A/B Test Simulation")
    st.caption(
        "Simulated experiment: does a retention offer reduce churn among "
        "high-risk customers?"
    )
    ...
    st.info(
        "**What this is:** A simulated A/B test demonstrating the methodology "
        "for validating retention interventions — randomization, control vs. "
        "treatment comparison, and significance testing. The treatment effect "
        "below is an **assumed input**, not an observed real-world outcome, "
        "since no live intervention data exists yet for this dataset."
    )
    ...
    with col1:
        threshold = st.slider(
            "At-risk threshold (churn probability)", 0.10, 0.90, THRESHOLD, 0.05
        )
    with col2:
        effect_pct = st.slider(
            "Assumed retention offer effect (% churn reduction)", 5, 40, 15, 5
        )
    ...
    if run_btn:
        ...
        at_risk = scored[scored["churn_probability"] > threshold].copy()

        np.random.seed(42)
        at_risk["group"] = np.random.choice(
            ["control", "treatment"], size=len(at_risk)
        )

        at_risk["adjusted_prob"] = at_risk["churn_probability"]
        at_risk.loc[at_risk["group"] == "treatment", "adjusted_prob"] *= (
            1 - effect_pct / 100
        )
        ...
        lift = (
            (control_rate - treatment_rate) / control_rate * 100
            if control_rate > 0
            else 0
        )
        ...
        r1, r2, r3, r4 = st.columns(4)
        r1.metric("At-Risk Customers", f"{len(at_risk):,}")
        r2.metric("Control Churn Rate", f"{control_rate*100:.1f}%")
        r3.metric("Treatment Churn Rate", f"{treatment_rate*100:.1f}%")
        r4.metric("Relative Reduction", f"{lift:.1f}%")

        chart_fig = px.bar(
            x=["Control", "Treatment"],
            y=[control_rate * 100, treatment_rate * 100],
            color=["Control", "Treatment"],
            color_discrete_map={"Control": "#888888", "Treatment": "#1D9E75"},
            labels={"x": "", "y": "Churn Rate (%)"},
            title="A/B Test: Retention Offer Impact on Churn",
        )
        ...
        if p_value < 0.05:
            st.success(
                ...
                f"difference detected between control and treatment groups."
            )
        else:
            st.warning(
                f"**p-value: {p_value:.4f}** — no statistically significant "
                f"difference detected."
            )

        st.caption(
            "Reminder: this p-value confirms the test correctly detects the "
            "assumed treatment effect above — it does not confirm that a real "
            "retention offer would achieve this exact reduction."
        )